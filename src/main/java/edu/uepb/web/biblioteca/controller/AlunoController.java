...
public class AlunoController {
	...
	public String homeAluno(@SessionAttribute("aluno") Aluno alunoLogado, Model model) {
		model.addAttribute("aluno", alunoLogado);
		model.addAttribute("listaEmprestimo", emprestimoService.getEmprestimoByAluno(getId()));
		model.addAttribute("listaDivida", dividaService.getListaDividaByAluno(getId()));
		model.addAttribute("listaReserva", reservaService.getListaReservaByAluno(getId()));
		return "homeAluno";
	}
	...
	public String alunoCadastraReserva(@ModelAttribute("reserva") Reserva reserva, Model model) {
		try {
			reservaService.reservaItem(reserva.getId(), reserva.getId(), reserva.isEmail());
	...
	public String renovarEmprestimo(@SessionAttribute("aluno") Aluno alunoLogado, @PathVariable("id") int idEmprestimo,
		...
		try {
			emprestimoService.renovarEmprestimo(emprestimo.getId(), emprestimo.getId());
		} catch (EmprestimoException e) {
			model.addAttribute("aluno", alunoLogado);
	...
	public String perfilAluno(@SessionAttribute("aluno") Aluno alunoLogado, Model model) {
		model.addAttribute("aluno", alunoService.getAlunoById(getId()));
		return "alunoPerfil";
	}
